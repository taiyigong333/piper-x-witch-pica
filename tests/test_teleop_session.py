...
from taiyi_piper_x_collect.writer import WriterReport
import taiyi_piper_x_collect.teleop_session as teleop_session
...
def test_teleop_example_can_be_loaded() -> None:
    ...
    assert "teleop_single_piper_x.launch.py" in config.controller.command
...
def test_external_teleop_starts_in_order_and_stops_process_groups(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "teleop_single_piper_x.launch.py" in commands[1]
    ...
    assert (tmp_path / "logs" / "piper_x_controller.log").exists()
