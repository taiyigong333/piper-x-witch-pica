...
from .piper_x import PiperXGripper, PiperXRobot
...
def create_robot(config: RobotConfig, pose_representation: str) -> RobotDevice:
    if config.driver == "piper_x":
        return PiperXRobot(config, pose_representation)
    if config.driver == "mock":
...
def create_gripper(config: GripperConfig, robot: RobotDevice | None = None) -> GripperDevice | None:
    ...
    if config.driver == "piper_x":
        if not isinstance(robot, PiperXRobot):
            raise ConfigurationError("Piper-X 原装夹爪必须复用对应的 PiperXRobot CAN 连接。")
        return PiperXGripper(robot)
