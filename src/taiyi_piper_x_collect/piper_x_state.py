...
def read_piper_x_state(config_path: str | Path, *, wait_s: float = 0.2) -> dict[str, Any]:
    """读取一帧 Piper-X 反馈，不发送使能或运动控制帧。"""

    if wait_s < 0:
        raise ConfigurationError("--wait-s 必须为非负数。")
    config = load_config(config_path)
    if config.robot.driver != "piper_x":
        raise ConfigurationError("读取 Piper-X 状态需要 robot.driver=piper_x。")
    robot = create_robot(config.robot, config.session.pose_representation)
    robot.start()
    try:
        # Piper-X 持续广播状态；短暂等待首帧 CAN 反馈。
        if wait_s:
            time.sleep(wait_s)
        state = robot.read()
    finally:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="只读输出 Piper-X 六关节角和 TCP 位姿")
    ...
    try:
        print(json.dumps(read_piper_x_state(args.config, wait_s=args.wait_s), ensure_ascii=False, indent=2))
        return 0
